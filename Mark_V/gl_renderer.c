...
#include "core_opengl.h"
	...
	void (APIENTRY *eglOrthoX) (GLfloat left, GLfloat right, GLfloat bottom, GLfloat top, GLfloat zNear, GLfloat zFar);
#else
	...
	void (APIENTRY *eglOrthoX) (GLdouble left, GLdouble right, GLdouble bottom, GLdouble top, GLdouble zNear, GLdouble zFar);
#endif
...
void (APIENTRY *eglFogfv) (GLenum pname, const GLfloat *params); // OpenGLES
...
void (APIENTRY *eglViewportX) (GLint x, GLint y, GLsizei width, GLsizei height);
...
//	eglNormal3f             = glNormal3f;
	eglOrthoX               = glOrthof;				// OpenGLES slight variant
...
//	eglReadBuffer           = NULL;					// Extension?  glReadBufferNV.  I see no evidence we need this for the basics.
...
void VID_Renderer_Set_OpenGL (void)
{
	...
	if (File_Exists (opengl32_dll_url)) {
		...
		//	eglBegin                = OPENGL_NOFUNCT(glBegin);
		...
		//	eglColor3f              = OPENGL_NOFUNCT(glColor3f);
		//	eglColor3fv             = OPENGL_NOFUNCT(glColor3fv);
		//	eglColor3ubv            = OPENGL_NOFUNCT(glColor3ubv);
			eglColor4f              = OPENGL_GETFUNC(glColor4f);
		//	eglColor4fv             = OPENGL_NOFUNCT(glColor4fv);
			eglColor4ub             = OPENGL_GETFUNC(glColor4ub);
		//	eglColor4ubv            = OPENGL_NOFUNCT(glColor4ubv);
		...
		//	eglDrawBuffer           = OPENGL_NOFUNCT(glDrawBuffer);
			eglEnable               = OPENGL_GETFUNC(glEnable);
	//		eglEnd                  = OPENGL_NOFUNCT(glEnd);
			eglFinish               = OPENGL_GETFUNC(glFinish);
			eglFogf                 = OPENGL_GETFUNC(glFogf);
			eglFogfv                = OPENGL_GETFUNC(glFogfv);
		//	eglFogi                 = OPENGL_NOFUNCT(glFogi);
		//	eglFogiv                = OPENGL_NOFUNCT(glFogiv);
		...
		//	eglGetTexImage          = OPENGL_NOFUNCT(glGetTexImage);
		...
		//	eglNormal3f             = OPENGL_GETFUNC(glNormal3f);
			eglOrthoX               = OPENGL_GETFUNC(glOrtho);
			eglPixelStorei			= OPENGL_GETFUNC(glPixelStorei);
		//	eglPolygonMode          = OPENGL_NOFUNCT(glPolygonMode);
			...
			//eglReadBuffer           = OPENGL_NOFUNCT(glReadBuffer);
		...
		//	eglTexCoord2f           = OPENGL_NOFUNCT(glTexCoord2f);
		//	eglTexCoord2fv          = OPENGL_NOFUNCT(glTexCoord2fv);
		...
		//	eglVertex2f             = OPENGL_NOFUNCT(glVertex2f);
		//	eglVertex2fv            = OPENGL_NOFUNCT(glVertex2fv);
		//	eglVertex3f             = OPENGL_NOFUNCT(glVertex3f);
		//	eglVertex3fv            = OPENGL_NOFUNCT(glVertex3fv);
			eglViewportX             = OPENGL_GETFUNC(glViewport);
...
#pragma message ("Linux/Mac compiles will have fun with following ...")
	eglAlphaFunc            = glAlphaFunc;
//	eglBegin                = glBegin;
	eglBindTexture          = glBindTexture;
	eglBlendFunc            = glBlendFunc;
...
//void eglFrustum (GLdouble left, GLdouble right, GLdouble bottom, GLdouble top, GLdouble zNear, GLdouble zFar)
//{
//	//eglRotatef	(90, 0, 0, 1); // Portrait rotate it.
//	//eglOrthoX		(left, right, bottom, top, zNear, zFar);
////	glLoadIdentity ();
//	eglRotatef		(90, 0, 0, 1); // Portrait rotate it.
//
//	//float left = (left + right) vid.screen.width - (vid.screen.width /*which is height for portrait*/ - _y);
//	//int y = x;					
//	//int width = _height;
//	//int height = _width;
//
////	eglOrthoX		(left *100, right *100, bottom*100, top*100, zNear, zFar);
//	eglFrustumX		(top, bottom, left, right, zNear, zFar);
//
//}
//

// Note: OpenGL doesn't know what a GLdouble is!  Keep that in mind!!
void eglOrtho	 (double left, double right, double bottom, double top, double zNear, double zFar)
{
	//eglRotatef		(90, 0, 0, 1); // Portrait rotate it.   // HEREON
	eglRotatef		(270, 0, 0, 1); // Portrait rotate it.   // HEREON
	eglOrthoX		(left, right, bottom, top, zNear, zFar);
}

void eglViewport (GLint _x, GLint _y, GLsizei _width, GLsizei _height)
{
//	int x = vid.screen.height - (_height /*which is width for portrait*/ + _y);  // HEREON
//	int y = _x;					
	int x = _y;
	int y = vid.screen.width - (_width + _x);
	int width = _height;
	int height = _width;
	
	eglViewportX (x, y, width, height);// * sysplat.content_scale, vid.screen.height * sysplat.content_scale);
}
...
void eglOrtho (GLdouble left, GLdouble right, GLdouble bottom, GLdouble top, GLdouble zNear, GLdouble zFar)
{
