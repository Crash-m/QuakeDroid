...
void R_DrawSpriteModel (entity_t *e)
{
	...
	egl_Begin_Textured_Solo_Quad_3D_Array (GL_TRIANGLE_FAN);
	//eglBegin (GL_TRIANGLE_FAN); // Handled was GL_QUADS, but changed to support r_showtris

	
	VectorMA (e->origin, sp_frame->down, s_up, point);
	VectorMA (point, sp_frame->left, s_right, point);
	egl_Add_Texco_VertexV_3D (0, sp_frame->tmax, point);
	...
	VectorMA (point, sp_frame->left, s_right, point);
	egl_Add_Texco_VertexV_3D (0, 0, point);
	//eglTexCoord2f (0, 0);
	//eglVertex3fv (point);
	...
	egl_Add_Texco_VertexV_3D (sp_frame->smax, 0, point);
	...
	VectorMA (e->origin, sp_frame->down, s_up, point);
	VectorMA (point, sp_frame->right, s_right, point);
	egl_Add_Texco_VertexV_3D (sp_frame->smax, sp_frame->tmax, point);
