...
void Sky_EmitSkyBoxVertex (float s, float t, int axis)
{
	...
	egl_Add_Texco_VertexV_3D (s, t, v);
	//eglTexCoord2f (s, t); eglVertex3fv (v);
}
...
void APIENTRY Sky_MH_DrawMultitexture (const float *v)
{
	...
	Sky_GetTexCoord (v, 16, &s2, &t2);

	egl_Add_MTexco_VertexV_3D (s, t, s2, t2, v);
	//renderer.GL_MTexCoord2fFunc (renderer.TEXTURE0, s, t);
	//renderer.GL_MTexCoord2fFunc (renderer.TEXTURE1, s2, t2);
	//eglTexCoord2f (s, t);
	//eglVertex3fv (v);
}
...
#define egl_Begin_Textured_QuadsX_3D_Elements(BEGINMODE, mtex)					egl_Elements_Reset(unused); ray_is_mtex = (mtex) // Ender
#define egl_End_Textured_QuadsX_3D_Elements() \
...
void Sky_MH_DrawPass (int axis, void (APIENTRY *Sky_DrawFunc) (const float *))
{
	...
	egl_Begin_Textured_QuadsX_3D_Elements (GL_QUADS, Sky_DrawFunc == Sky_MH_DrawMultitexture);
	//eglBegin (GL_QUADS); // handled
	...
	for (i = 0; i < di; i++) {
		for (j = 0; j < dj; j++) {
			...
			if (Sky_DrawFunc) {
				...
				// Baker: textures aren't actually enabled for this, just put out something fake for now ...
				egl_Add_Texco_VertexV_3D (0, 0, vec[0]);
				egl_Add_Texco_VertexV_3D (0, 1, vec[1]);
				egl_Add_Texco_VertexV_3D (1, 0, vec[2]);
				egl_Add_Texco_VertexV_3D (1, 1, vec[3]);
			}

			//alert ("%d", ray_is_mtex);
			
			egl_Complete_Textured_Quad_3D_Element ();
	...
	egl_End_Textured_QuadsX_3D_Elements (); //egl_End_Textured_Quads_3D_Elements ();
	//	glEnableClientState		(GL_TEXTURE_COORD_ARRAY);													\
	//glEnableClientState		(GL_VERTEX_ARRAY);															\
	//glTexCoordPointer		(2 /*dimension*/, GL_FLOAT, /*stride*/ sizeof(floatsamx_t), &_ray_base[0].s); \
	//if (ray_is_mtex) { /*switch to mtex and enable */													\
