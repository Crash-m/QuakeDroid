...
void DrawGLPoly (glpoly_t *p, int renderfx)
{
	...
	{
		egl_Begin_Textured_Polygon_3D_Array (GL_POLYGON);
		...
		for (i=0 ; i<p->numverts ; i++, v+= VERTEXSIZE)
		{
			...
			egl_Add_Texco_VertexV_3D (s0, t0, v);
			//eglTexCoord2f (s0, t0);
			//eglVertex3fv (v);
		}
		egl_End_Textured_Polygon_3D_Array ();
		//eglEnd ();
		return;
	}
	...
	if ((renderfx & SURF_DRAWTURB) && (renderfx & SURF_DRAWTELE)==0 && gl_waterripple.value) // liquid, not a tele and with water ripple
	{
		...
		for (i=0 ; i<p->numverts ; i++, v+= VERTEXSIZE)
		{
			...
			egl_Add_Texco_VertexV_3D (v[3], v[4], newverts);
	...
	for (i=0 ; i<p->numverts ; i++, v+= VERTEXSIZE)
	{
		egl_Add_Texco_VertexV_3D (v[3], v[4], v);
		//eglTexCoord2f (v[3], v[4]);
		//eglVertex3fv (v);
...
void DrawGLTriangleFan (glpoly_t *p, int renderfx)
{
	...
	if ((renderfx & SURF_DRAWTURB) && (renderfx & SURF_DRAWTELE)==0 && gl_waterripple.value)
	{
		...
		egl_Begin_Untextured_Triangle_Fan_3D_Array (GL_TRIANGLE_FAN);
		v = p->verts[0];
		for (i=0 ; i<p->numverts ; i++, v+= VERTEXSIZE)
		{
			vec3_t		newverts;
			...
			egl_Add_VertexV_3D (v);
			//eglVertex3fv (newverts);
		}
		egl_End_Untextured_Triangle_Fan_3D_Array ();
		//eglEnd ();
		return;
	}
	...
	if (r_drawflat_cheatsafe)
	{
		if ((s->flags & SURF_DRAWTURB) && frame.oldwater)
		{
			for (p = s->polys->next; p; p = p->next)
			{
				...
				eglColor3f (rand()%256/255.0, rand()%256/255.0, rand()%256/255.0);GL_ErrorPrint;
...
// fullbright
	if ((r_fullbright_cheatsafe) && !(s->flags & SURF_DRAWTILED))
	{
		if (entalpha < 1)
		{
			...
			eglColor4f(1, 1, 1, entalpha);GL_ErrorPrint;
		}
		if (s->flags & SURF_DRAWFENCE) {
			eglEnable (GL_ALPHA_TEST); GL_ErrorPrint;// Flip on alpha test
		}
		...
		rs_brushpasses++;
		if (s->flags & SURF_DRAWFENCE) {
			eglDisable (GL_ALPHA_TEST); GL_ErrorPrint;// Flip alpha test back off
		}
...
// r_lightmap
	if (r_lightmap_cheatsafe)
	{
		...
		{
			eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);GL_ErrorPrint;
			eglColor3f(0.5, 0.5, 0.5);GL_ErrorPrint;
		}
		...
		{
			egl_Add_Texco_VertexV_3D (v[5], v[6], v);
...
#define GL_SOURCE1_ALPHA_ARB 0x8589
#endif 
			eglTexEnvi(GL_TEXTURE_ENV, GL_SOURCE0_ALPHA_ARB, GL_PRIMARY_COLOR); GL_ErrorPrint;
			eglTexEnvi(GL_TEXTURE_ENV, GL_SOURCE1_ALPHA_ARB, GL_TEXTURE);GL_ErrorPrint;
			eglTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_ALPHA, GL_MODULATE);GL_ErrorPrint;

			GL_EnableMultitexture(); // selects TEXTURE1
			GL_Bind (lightmap[s->lightmaptexturenum].texture);
			R_RenderDynamicLightmaps (s);
			eglTexEnvi(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_COMBINE);GL_ErrorPrint;
			eglTexEnvi(GL_TEXTURE_ENV, GL_COMBINE_RGB, GL_MODULATE);GL_ErrorPrint;
			
			eglTexEnvi(GL_TEXTURE_ENV, GL_SRC0_RGB, GL_PREVIOUS);GL_ErrorPrint;
			eglTexEnvi(GL_TEXTURE_ENV, GL_SRC0_RGB, GL_PREVIOUS);GL_ErrorPrint;
			eglTexEnvi(GL_TEXTURE_ENV, GL_SRC1_RGB, GL_TEXTURE);GL_ErrorPrint;
			eglTexEnvf(GL_TEXTURE_ENV, GL_RGB_SCALE, 2.0f);GL_ErrorPrint;
			egl_Begin_MTextured_Polygon_3D_Array (GL_POLYGON);
			//eglBegin(GL_POLYGON);  // mtex // not handled
			v = s->polys->verts[0];
			for (i=0 ; i<s->polys->numverts ; i++, v+= VERTEXSIZE)
			{
		...
		else //case 3: texture in one pass, lightmap in second pass using 2x modulation blend func, fog in third pass
		{
			...
			eglDepthMask (GL_FALSE);GL_ErrorPrint;
			eglEnable (GL_BLEND);GL_ErrorPrint;
			eglBlendFunc(GL_DST_COLOR, GL_SRC_COLOR); GL_ErrorPrint; //2x modulate
			Fog_StartAdditive ();
			...
			if (Fog_GetDensity() > 0)
			{
				eglBlendFunc(GL_ONE, GL_ONE); GL_ErrorPrint;//add
				eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);GL_ErrorPrint;
				eglColor3f(0,0,0);GL_ErrorPrint;
				DrawGLPoly (s->polys, s->flags);GL_ErrorPrint;
				eglColor3f(1,1,1);GL_ErrorPrint;
				eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);GL_ErrorPrint;
				rs_brushpasses++;
			}
	...
	else
	{
		if (renderer.gl_mtexable) //case 4: texture and lightmap in one pass, regular modulation
		{
			...
			eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);GL_ErrorPrint;
			egl_Begin_MTextured_Polygon_3D_Array (GL_POLYGON);
			//eglBegin(GL_POLYGON); // handled mtex
			v = s->polys->verts[0];
			for (i=0 ; i<s->polys->numverts ; i++, v+= VERTEXSIZE) {
				if (s->flags & (SURF_DRAWENVMAP | SURF_SCROLLX | SURF_SCROLLY))
				{
					...
					egl_Add_MTexco_VertexV_3D (v[5], v[6], v[5], v[6], v);
					//renderer.GL_MTexCoord2fFunc (renderer.TEXTURE0, v[3], v[4]);
					//renderer.GL_MTexCoord2fFunc (renderer.TEXTURE1, v[5], v[6]);
					//eglVertex3fv (v);
		...
		else //case 6: texture in one pass, lightmap in a second pass, fog in third pass
		{
			...
			eglDepthMask (GL_FALSE);GL_ErrorPrint;
			eglEnable (GL_BLEND);GL_ErrorPrint;
			eglBlendFunc (GL_ZERO, GL_SRC_COLOR); GL_ErrorPrint;//modulate
			Fog_StartAdditive ();
...
fullbrights:
	if (gl_fullbrights.value && t->fullbright)
	{
		eglDepthMask (GL_FALSE);GL_ErrorPrint;
		eglEnable (GL_BLEND);GL_ErrorPrint;
		eglBlendFunc (GL_ONE, GL_ONE);GL_ErrorPrint;
		eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);GL_ErrorPrint;
		eglColor3f (entalpha, entalpha, entalpha);GL_ErrorPrint;
