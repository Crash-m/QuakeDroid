...
//this macro is in sample size workspace coordinates
	...
	// April 6 2018 - Rendered upside down so can't use textured quad 2d emit.
	#define sGL_Bloom_SamplePass(xpos, ypos)                            \
		egl_Begin_Textured_Solo_Quad_2D_Array (GL_QUADS);				\
		egl_Add_Texco_Vertex_2D	(0, glt_bloom_screen_downsample_height_tch, xpos, ypos);		\
		egl_Add_Texco_Vertex_2D	(0, 0, xpos, ypos + g_sample_height);							\
		egl_Add_Texco_Vertex_2D	(glt_bloom_screen_downsample_width_tcw, 0, xpos + g_sample_width, ypos + g_sample_height); \
		egl_Add_Texco_Vertex_2D	(glt_bloom_screen_downsample_width_tcw, glt_bloom_screen_downsample_height_tch, xpos + g_sample_width, ypos); \
...
//		eglEnd();
	...
	#define sGL_Bloom_Quad(x, y, width, height, textwidth, textheight)	\
		egl_Begin_Textured_Solo_Quad_2D_Array (GL_QUADS);				\
		egl_Add_Texco_Vertex_2D	(0, textheight, x, y);					\
		egl_Add_Texco_Vertex_2D	(0, 0, x, y + height);					\
		egl_Add_Texco_Vertex_2D	(textwidth, 0, x + width, y + height);	\
		egl_Add_Texco_Vertex_2D	(textwidth, textheight, x + width, y);	\
		egl_End_Textured_Solo_Quad_2D_Array ();
...
//  eglBlendFunc	(GL_ONE, GL_ONE);
	...
	egl_Begin_Textured_Solo_Quad_2D_Array (GL_QUADS);
    egl_Add_Texco_Vertex_2D	(0, glt_bloom_screen_downsample_height_tch, curView_x, curView_y);      
    egl_Add_Texco_Vertex_2D	(0, 0, curView_x, curView_y + curView_height);  
    egl_Add_Texco_Vertex_2D	(glt_bloom_screen_downsample_width_tcw, 0, curView_x + curView_width, curView_y + curView_height);  
    egl_Add_Texco_Vertex_2D	(glt_bloom_screen_downsample_width_tcw, glt_bloom_screen_downsample_height_tch, curView_x + curView_width, curView_y);
	egl_End_Textured_Solo_Quad_2D_Array ();
