...
#define DRAW_PARTICLE_BILLBOARD(_ptex, _p, _coord)															\
	...
	eglColor4ubv (_p->color);																				\
																											\
	egl_Begin_Textured_Solo_Quad_3D_Array (GL_QUADS);														\
	egl_Add_Texco_VertexV_3D (_ptex->coords[_p->texindex][0], ptex->coords[_p->texindex][3], _coord[0]);	\
	egl_Add_Texco_VertexV_3D (_ptex->coords[_p->texindex][0], ptex->coords[_p->texindex][1], _coord[1]);	\
	egl_Add_Texco_VertexV_3D (_ptex->coords[_p->texindex][2], ptex->coords[_p->texindex][1], _coord[2]);	\
	egl_Add_Texco_VertexV_3D (_ptex->coords[_p->texindex][2], ptex->coords[_p->texindex][3], _coord[3]);	\
	egl_End_Textured_Solo_Quad_3D_Array ();																	\
	eglPopMatrix (); mglPopStates (); // Ender
...
//			assert (in_range (0, pt->texture_numbp, ARRAY_COUNT(particle_textures) - 1));
		...
		case pd_spark:
			...
			for (p = pt->start ; p ; p = p->next)
			{
				...
				egl_Begin_Untextured_RGBA_Triangle_Fan_3D_Array (GL_TRIANGLE_FAN);
				//eglBegin (GL_TRIANGLE_FAN); // handled

				
				egl_Add_RGBAUBV_VertexV_3D_Array (p->color, p->org);
				//eglColor4ubv (p->color);
				//eglVertex3fv (p->org);
				...
				for (j=7 ; j>=0 ; j--)
				{
					...
					egl_Add_RGBAUB_VertexV_3D_Array ((GLubyte)(p->color[0] >> 1), (GLubyte)(p->color[1] >> 1), (GLubyte)(p->color[2] >> 1), 0, v);
					//eglVertex3fv (v);
				}
				
				egl_End_Untextured_RGBA_Triangle_Fan_3D_Array ();
				//eglEnd ();
			}
			break;
		...
		case pd_sparkray:
			...
			for (p = pt->start ; p ; p = p->next)
			{
				...
				egl_Add_RGBAUBV_VertexV_3D_Array (p->color, p->endorg);
				//eglColor4ubv (p->color);
