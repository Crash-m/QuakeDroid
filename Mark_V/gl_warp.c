...
void DrawWaterPoly (glpoly_t *p, cbool isteleporter)
{
	...
	if (load_subdivide_size > 48)
	{
		...
		for (i = 0 ;  i <p->numverts ; i++, v+= VERTEXSIZE)
		{
			...
			if (gl_waterripple.value && isteleporter == false)
			{
				...
				egl_Add_Texco_VertexV_3D (WARPCALC2(v[3],v[4]), WARPCALC2(v[4],v[3]), newverts);
				//eglVertex3fv (newverts);
			}
			else {
				egl_Add_Texco_VertexV_3D (WARPCALC2(v[3],v[4]), WARPCALC2(v[4],v[3]), v);
				//eglVertex3fv (v);
			}
	...
	{
		egl_Begin_Texco_Vertex_3D_Array (GL_POLYGON);
		//eglBegin (GL_POLYGON); // handled
		v = p->verts[0];
		for (i=0 ; i<p->numverts ; i++, v+= VERTEXSIZE)
		{
...
void R_SetupView_UpdateWarpTextures (void)
{
	...
	for (i=0; i<cl.worldmodel->numtextures; i++)
	{
		...
		{
			x2 = x + warptess;
			egl_Begin_Textured_Triangle_Strip_2D_Array (GL_TRIANGLE_STRIP);
			//eglBegin (GL_TRIANGLE_STRIP); // handled

			for (y = 0.0; y < 128.01; y += warptess) // .01 for rounding errors
			{
				egl_Add_Texco_Vertex_2D (WARPCALC(x, y), WARPCALC(y, x), x, y);
				// eglTexCoord2f (WARPCALC(x,y), WARPCALC(y,x));
				// eglVertex2f (x,y);

				egl_Add_Texco_Vertex_2D (WARPCALC(x2,y), WARPCALC(y,x2), x2, y);
				// eglTexCoord2f (WARPCALC(x2,y), WARPCALC(y,x2));
				// eglVertex2f (x2,y);
			}

			egl_End_Textured_Triangle_Strip_2D_Array ();
			//eglEnd();
		}
...
static void GL_Warp_CalcUnderwaterCoords (float x, float y, float CYCLE_X, float CYCLE_Y, float AMP_X, float AMP_Y)
{
	...
	egl_Add_Texco_Vertex_2D (texX, texY, x, y);
