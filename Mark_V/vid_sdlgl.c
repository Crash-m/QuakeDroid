...
#pragma message ("If this event fires in Android, does it work?")
#pragma message ("FIXUS 0: See if this works")
#if PLATFORM_SCREEN_PORTRAIT !=0
...
static SDL_DisplayMode *VID_SDL2_GetDisplayMode(int width, int height, int bpp)
{
	...
	{
		if (SDL_GetDisplayMode(0, i, &mode) == 0
#if 1// PLATFORM_SCREEN_PORTRAIT != 0
...
// Ok, my platform neutral way does not appear to be optimal for SDL so am going to try it more like the "Quakespasm" way, which is more compatible with the SDL philosophy.
	...
	//alert ("VID_Local_SetMode %d %d is first? %d", p->width, p->height, first);
#if 1
	...
	if (!sysplat.mainwindow) {
		...
		sysplat.mainwindow = SDL_CreateWindow(ENGINE_NAME, SDL_WINDOWPOS_CENTERED, SDL_WINDOWPOS_CENTERED, 
	#if PLATFORM_SCREEN_PORTRAIT !=0
			p->height, p->width,
	#else
			p->width, p->height,
	...
	#if PLATFORM_SCREEN_PORTRAIT !=0
		SDL_SetWindowDisplayMode (sysplat.mainwindow, /*seriously?? */ VID_SDL2_GetDisplayMode(p->height, p->width, vid.desktop.bpp));
	#else
...
#pragma message ("FIXUS 1: SDL_GL_SetSwapInterval commented out")
	//SDL_GL_SetSwapInterval (!!vid_vsync.value);
...
#pragma message ("FIXUS 2: Can we comment out the makecurrent")
	SDL_GL_MakeCurrent (sysplat.mainwindow, sysplat.draw_gl_context);
	//alert ("Made current %d %d", (int)sysplat.mainwindow, (int)sysplat.draw_gl_context);
	return true;
...
#if 0 && PLATFORM_SCREEN_PORTRAIT !=0
		SDL_SetWindowSize (sysplat.mainwindow, p->height, p->width);
#else
        SDL_SetWindowSize (sysplat.mainwindow, p->width, p->height);
#endif // !PLATFORM_SCREEN_PORTRAIT
