...
void GL_DrawAliasFrame_QMB_Flame_And_Shadows (aliashdr_t *paliashdr, lerpdata_t lerpdata, int commands_offset, cbool truncate_flame, float mdl_entalpha)
{
	...
	while (1)
	{
		...
		{
			count = -count;
			egl_Begin_Textured_ColorYNf_TriangleX_3D_Array (GL_TRIANGLE_FAN, mtexenabled, shading);
			//eglBegin (GL_TRIANGLE_FAN); // handled
		}
		else {
			egl_Begin_Textured_ColorYNf_TriangleX_3D_Array (GL_TRIANGLE_STRIP, mtexenabled, shading);
			//eglBegin (GL_TRIANGLE_STRIP); // handled
		}
		...
		do
		{
			...
			if (shading) {
				if (r_drawflat_cheatsafe) {
					...
					vertcolor[0] = rand()%256/255.0;
					vertcolor[1] = rand()%256/255.0;
					vertcolor[2] = rand()%256/255.0;
			...
			if (lerping) {
				...
				if (mtexenabled) {
					if (shading) {
						egl_Add_MTexco_RGBAvf_Vertex_3D (u, v, u, v, vertcolor, 
							verts1->v[0]*iblend + verts2->v[0]*blend, 
							verts1->v[1]*iblend + verts2->v[1]*blend, 
							verts1->v[2]*iblend + verts2->v[2]*blend
						);
					} else {
						egl_Add_MTexco_Vertex_3D (u, v, u, v, 
							verts1->v[0]*iblend + verts2->v[0]*blend, 
							verts1->v[1]*iblend + verts2->v[1]*blend, 
							verts1->v[2]*iblend + verts2->v[2]*blend
						);
					...
					if (shading) {
						egl_Add_Texco_RGBAvf_Vertex_3D (u, v, vertcolor, 
							verts1->v[0]*iblend + verts2->v[0]*blend, 
							verts1->v[1]*iblend + verts2->v[1]*blend, 
							verts1->v[2]*iblend + verts2->v[2]*blend
						);
					} else {
						egl_Add_Texco_Vertex_3D (u, v,  
							verts1->v[0]*iblend + verts2->v[0]*blend, 
							verts1->v[1]*iblend + verts2->v[1]*blend, 
							verts1->v[2]*iblend + verts2->v[2]*blend
						);
				...
				if (mtexenabled) {
					if (shading) {
						egl_Add_MTexco_RGBAvf_VertexV_3D (u, v, u, v, vertcolor, verts1->v);
					}
					else {
						egl_Add_MTexco_VertexV_3D (u, v, u, v, verts1->v);
					}
				} else {
					if (shading) {
						egl_Add_Texco_RGBAvf_VertexV_3D (u, v, vertcolor, verts1->v);
					}
					else {
						egl_Add_Texco_VertexV_3D (u, v, verts1->v);
					}
				}
		...
		egl_End_Textured_ColorYNf_TriangleX_3D_Array ();
		//eglEnd ();
	}
...
// commands_offset, cbool truncate_flame, float mdl_entalpha are not used in this
	...
	while (1)
	{
		...
		do
		{
			if (mtexenabled)
			{
				...
				if (shading) {
					egl_Add_MTexco_RGBAvf_VertexV_3D (lv->texcoord[0], lv->texcoord[1], lv->texcoord[0], lv->texcoord[1], lv->color, lv->position);
				} else { 
					egl_Add_MTexco_VertexV_3D (lv->texcoord[0], lv->texcoord[1], lv->texcoord[0], lv->texcoord[1], lv->position);
				}
			}
			else {
				//eglTexCoord2fv (lv->texcoord);
				if (shading) {
					egl_Add_Texco_RGBAvf_VertexV_3D (lv->texcoord[0], lv->texcoord[1], lv->color, lv->position);
				} else {
					egl_Add_Texco_VertexV_3D (lv->texcoord[0], lv->texcoord[1], lv->position);
				}
			}
		...
		//alert ("color %d mtex %d", ray_is_color, ray_is_mtex);
		egl_End_Textured_ColorYNf_TriangleX_3D_Array ();
		//eglEnd ();
	}
...
//#if 1
		...
		for (i = 0; i < r_num_lerped_verts; i++)
		{
			...
			for (j = 0; j < c; j++)
			{
				...
				if (hit[1] == 0)
				{
					egl_Add_Untextured_QuadV_3D_Element (
						r_lerped_verts[i + r_num_lerped_verts].position,
						r_lerped_verts[i2 + r_num_lerped_verts].position,
						r_lerped_verts[i2].position,
						r_lerped_verts[i].position
					);
				}
			}
		}
...
void RB_ShadowVolumeFinish (void)
{
	...
	egl_Add_Vertex_3D (-100, 100, -10);
	egl_Add_Vertex_3D (100, 100, -10);
	egl_Add_Vertex_3D (100, -100, -10);
	egl_Add_Vertex_3D (-100, -100, -10);
	egl_End_Untextured_Solo_Quad_3D_Array ();
...
void R_DrawAliasModel_ShowTris (entity_t *e)
{
	...
	//GL_DrawAliasFrame_New (paliashdr, lerpdata, paliashdr->commands, e->is_fake_frame0, 1 /*entalpha*/);
	eglPopMatrix ();
}
