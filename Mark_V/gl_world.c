...
//#define DRAW_ACTIVE_MIRROR (active_mirror && frame.in_mirror_overlay)
			...
			if (s->flags & SURF_DRAWFENCE)
			{
				...
				eglEnable (GL_ALPHA_TEST); GL_ErrorPrint; // Flip alpha test back on
				GL_EnableMultitexture(); // selects TEXTURE1
			}
			...
			if (active_mirror) {
				...
				// https://www.gamedev.net/forums/topic/83634-multitexture-lightmaps/
				...
				if (!s->culled || frame.in_mirror_draw) // By definition, water cannot be mirror.
				{
					if (!bound) //only bind once we are sure we need this texture
					{
						...
						if (texture_alpha < 1)
						{
							...
							eglColor4f (1,1,1, texture_alpha); GL_ErrorPrint;
						}
						else
						{
...
void R_DrawWorld (void)
{
	...
	if (gl_overbright.value)
	{
		if (renderer.gl_texture_env_combine && renderer.gl_mtexable)
		{
			...
			GL_EnableMultitexture ();
			eglTexEnvf(GL_TEXTURE_ENV, GL_RGB_SCALE, 1.0f); GL_ErrorPrint;
			eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE); GL_ErrorPrint;
			GL_DisableMultitexture ();
			eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE); GL_ErrorPrint;
		}
