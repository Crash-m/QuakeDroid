...
void Fog_SetupFrame (void)
{
	...
	eglFogfv(GL_FOG_COLOR, Fog_GetColor(&startfogdist, &endfogdist));  GL_ErrorPrint;
#if 1
	if (fogex)
	{
		...
		eglFogi(GL_FOG_MODE, GL_LINEAR);  GL_ErrorPrint;
		eglFogf(GL_FOG_START, startfogdist);  GL_ErrorPrint;
		eglFogf(GL_FOG_END, endfogdist);  GL_ErrorPrint;
...
//		eglFogf(GL_FOG_END, endfogdist);
	...
	{
		eglFogi(GL_FOG_MODE, GL_EXP2);  GL_ErrorPrint;
		eglFogf(GL_FOG_DENSITY, Fog_GetDensity() / 64.0);  GL_ErrorPrint;
	}
}
...
{
	if (Fog_GetDensity() > 0)
		eglEnable(GL_FOG);  GL_ErrorPrint;
}
...
{
	if (Fog_GetDensity() > 0)
		eglDisable(GL_FOG);  GL_ErrorPrint;
}
...
void Fog_StartAdditive (void)
{
	...
	if (Fog_GetDensity() > 0) {
		eglFogfv(GL_FOG_COLOR, color);  GL_ErrorPrint;
	}
}
...
{
	if (Fog_GetDensity() > 0) {
		eglFogfv(GL_FOG_COLOR, Fog_GetColor(NULL, NULL));   GL_ErrorPrint;
	}
}
