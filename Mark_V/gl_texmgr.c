...
#if PLATFORM_MOBILE
	const int	gl_solid_format = GL_RGBA; //RGB_3;
	const int	gl_alpha_format = GL_RGBA; //RGBA_4;
#else
...
static void TexMgr_SetFilterModes (gltexture_t *glt)
{
	...
	{
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_NEAREST);  GL_ErrorPrint;
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_NEAREST);   GL_ErrorPrint;
	}
	else if (glt->flags & TEXPREF_LINEAR)
	{
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, GL_LINEAR);  GL_ErrorPrint;
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, GL_LINEAR);  GL_ErrorPrint;
	}
	else if (glt->flags & TEXPREF_MIPMAP)
	{
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, glmodes[glmode_idx].magfilter);  GL_ErrorPrint;
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, glmodes[glmode_idx].minfilter);  GL_ErrorPrint;
	}
	else
	{
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAG_FILTER, glmodes[glmode_idx].magfilter);  GL_ErrorPrint;
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MIN_FILTER, glmodes[glmode_idx].magfilter);  GL_ErrorPrint;
	}

#if PLATFORM_MOBILE == 0 // NOT
	// Baker: April 24 2018 - I can't get GL_TEXTURE_MAX_ANISOTROPY_EXT to work on my device
	eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, gl_texture_anisotropy.value);   GL_ErrorPrint;
...
{
#if PLATFORM_MOBILE == 0 // NOT
// Baker: April 23 2018 - I can't get gl_texture_anisotropy to work.
// It may just be my Android device doesn't support it.
...
{
#if 0 //PLATFORM_MOBILE == 0 // NOT.  This cannot be done on OpenGLES
...
gltexture_t *TexMgr_NewTexture (void)
{
	...
	eglGenTextures(1, &glt->texnum); GL_ErrorPrint;
	numgltextures++;
	return glt;
}
...
gltexture_t* TexMgr_FreeTexture (gltexture_t *kill)
{
	...
	if (active_gltextures == kill)
	{
		...
		eglDeleteTextures(1, &kill->texnum); GL_ErrorPrint;
		numgltextures--;
		return NULL;
	}
...
#if 0
			if (GL_ErrorPrint) {
				alert ("miplevel %d internalformat %d, mipwidth %d, mipheight %d", 0, gl_solid_format, gl_warpimagesize, gl_warpimagesize);
			}
#endif
...
#if 0 // PLATFORM_MOBILE != 0 // NOY
	if (GL_ErrorPrint) {
		alert ("vid.maxwarpwidth, vid.maxwarpheight %d %d", vid.maxwarpwidth, vid.maxwarpheight);
	}
#endif
}
...
#ifdef PLATFORM_ANDROID // May 7 2018 - Baker: Ugly temp
	if (1) {
		...
		//msgbox (NULL, "Pushing gamma2 %2.2f vid_hardwaregamma is %g", vid_gamma.value, vid_hardwaregamma.value/**/);
	}
#endif // PLATFORM_ANDROID // GLQUAKE + !DIRECT3D9_WRAPPER
...
//						i=i;
		...
		//__android_log_print (ANDROID_LOG_INFO, CORE_ANDROID_LOG_TAG,
		//	"%s %d %d fmt internalformat %d",
		//		glt->name,
		//		(int)glt->width,
		//		(int)glt->height,
		//		(int)internalformat);

		eglTexImage2D (GL_TEXTURE_2D, 0, internalformat, glt->width, glt->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, myData);   GL_ErrorPrint;
		...
		if (Flag_Check_Bool(glt->flags, TEXPREF_MIPMAP) && vid.direct3d != 9) // MH says DX9 NO MIPMAP doesn't need to mipmap.
		{
			...
			for (miplevel = 1; mipwidth > 1 || mipheight > 1; miplevel++)
			{
				...
				eglTexImage2D (GL_TEXTURE_2D, miplevel, internalformat, mipwidth, mipheight, 0, internalformat, GL_UNSIGNED_BYTE, myData);
#if 0
				if (GL_ErrorPrint) {
					alert ("miplevel %d internalformat %d, mipwidth %d, mipheight %d", miplevel, internalformat, mipwidth, mipheight);
				}
#endif
			}
		}
...
static void TexMgr_LoadLightmap (gltexture_t *glt, byte *data)
{
	...
	eglTexImage2D (GL_TEXTURE_2D, 0, GL_RGBA, glt->width, glt->height, 0, GL_RGBA, GL_UNSIGNED_BYTE, data);  GL_ErrorPrint;
...
void TexMgr_DeleteImages (cbool gamma_texture_only)
{
	...
	for (glt=active_gltextures, count = 0; glt; glt = glt->next, count ++)
	{
		eglDeleteTextures(1, &glt->texnum); GL_ErrorPrint;
	}
}
...
void GL_Bind (gltexture_t *texture)
{
	...
	if ((int)texture->texnum != currenttexture)
	{
		...
		eglBindTexture (GL_TEXTURE_2D, currenttexture);  GL_ErrorPrint;
