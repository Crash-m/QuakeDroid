...
vmode_t VID_Cvars_To_Mode (void)
{
	...
	retmode.height	= (int)vid_width.value;
    retmode.width	= (int)vid_height.value;
#else
...
// These must all be read.
	...
	if (1) {
		//msgbox (NULL, "Reading %2.2f vid_hardwaregamma is %g", vid_gamma.value, vid_hardwaregamma.value/**/);
	}
