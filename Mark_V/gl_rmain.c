...
{
	eglTranslatef (origin[0],  origin[1],  origin[2]);  GL_ErrorPrint;
	eglRotatef (angles[1],  0, 0, 1);  GL_ErrorPrint;
	eglRotatef (-angles[0],  0, 1, 0);  GL_ErrorPrint;
	eglRotatef (angles[2],  1, 0, 0);  GL_ErrorPrint;
}
...
void GL_PolygonOffset (int offset)
{
	if (offset > 0)
	{
		...
		eglPolygonOffset(1, offset);  GL_ErrorPrint;
	}
	else if (offset < 0)
	{
#ifndef PLATFORM_ANDROID
...
static glmatrix exo_matrix_m = {0};
static glmatrix exo_matrix2_m = {0};
static void R_RenderScene_SetupScene_SetupGL_SetFrustum(float fovx, float fovy)
{
...
#if PLATFORM_SCREEN_PORTRAIT != 0
	{
		...
		Mat4_Identity_Set (&exo_matrix_m);
		//Mat4_Rotate (&exo_matrix_m, 90, 0, 0, 1); // HEREON
		Mat4_Rotate (&exo_matrix_m, 270, 0, 0, 1); // HEREON
		Mat4_Frustum (&exo_matrix_m, -xmax + frustum_skew, xmax + frustum_skew, -ymax, ymax, (focus0.znear = NEARCLIP_1), (focus0.zfar = gl_farclip.value));

		eglLoadMatrixf (exo_matrix_m.m16);
		
		
		Mat4_Identity_Set (&exo_matrix2_m);
		Mat4_Frustum (&exo_matrix2_m, -xmax + frustum_skew, xmax + frustum_skew, -ymax, ymax, (focus0.znear = NEARCLIP_1), (focus0.zfar = gl_farclip.value));
		
		
	}
#else
	eglFrustum(-xmax + frustum_skew, xmax + frustum_skew, -ymax, ymax, (focus0.znear = NEARCLIP_1), (focus0.zfar = gl_farclip.value));  GL_ErrorPrint;
#endif // !PORTRAIT
...
static void R_RenderScene_SetupScene_SetupGL (void)
{
	...
	eglMatrixMode(GL_PROJECTION);  GL_ErrorPrint;
		eglLoadIdentity ();  GL_ErrorPrint;

		if (frame.do_glwarp)
		{
			// this will go down into sbar territory so refresh it
			eglViewport (0, 0, vid.maxwarpwidth, vid.maxwarpheight);  GL_ErrorPrint;
			Sbar_Changed ();
		}
		else {
			eglViewport (clx + r_refdef.vrect.x,
					...
					r_refdef.vrect.height);  GL_ErrorPrint;
			...
			eglGetIntegerv (GL_VIEWPORT, focus0.game_viewport);  GL_ErrorPrint;
#endif
		}
		//johnfitz
	...
	eglGetFloatv (GL_PROJECTION_MATRIX, r_projection_matrix);   GL_ErrorPrint;// Store off the model view matrix.  (r_world_matrix)
	if (!frame.do_glwarp) {
#if PLATFORM_SCREEN_PORTRAIT !=0
		Mat4_Copy (&focus0.game_projection, &exo_matrix2_m);
#else
	...
	eglMatrixMode(GL_MODELVIEW);  GL_ErrorPrint;
		eglLoadIdentity ();  GL_ErrorPrint;

		eglRotatef (-90,  1, 0, 0);	     GL_ErrorPrint; // put Z going up
		eglRotatef (90,  0, 0, 1);  GL_ErrorPrint;	    // put Z going up
		eglRotatef (-r_refdef.viewangles[2],  1, 0, 0);  GL_ErrorPrint;
		eglRotatef (-r_refdef.viewangles[0],  0, 1, 0);  GL_ErrorPrint;
		eglRotatef (-r_refdef.viewangles[1],  0, 0, 1);  GL_ErrorPrint;
		eglTranslatef (-r_refdef.vieworg[0],  -r_refdef.vieworg[1],  -r_refdef.vieworg[2]);  GL_ErrorPrint;
	...
	//
	if (gl_cull.value) {
		eglEnable(GL_CULL_FACE);   GL_ErrorPrint;
	}
	else {
		eglDisable(GL_CULL_FACE);   GL_ErrorPrint;
	}

	// Make sure these guys are properly set in mirrors
	eglDisable(GL_BLEND);  GL_ErrorPrint;
	eglDisable(GL_ALPHA_TEST);  GL_ErrorPrint;
	eglEnable(GL_DEPTH_TEST);  GL_ErrorPrint;
}
...
//	if (renderer.gl_stencilbits) // Clear buffer upon any function needing to use it.  Per frame is irrelevant.
	...
	eglClear (clearbits);  GL_ErrorPrint;


	eglDepthRange (Q_GLDEPTHRANGE_MIN_0_0, Q_GLDEPTHRANGE_MAX_0_5);  GL_ErrorPrint;
...
// Setup per frame variables
	frame.oldwater = (PLATFORM_MOBILE || gl_oldwater.value || (vid.direct3d == 8) ); // OpenGLES 1.1 hates oldwater
...
// current viewleaf
	...
	if (r_waterwarp.value)
	{
		...
		{
			// (Mobile) OpenGLES hates the sophisticated twisty waterwarp
			if (!PLATFORM_MOBILE && r_waterwarp.value <= 1 && vid.direct3d !=8 ) // dx8 has no CopyTexSubImage, therefore cannot do WinQuake R_WATERWARP
...
void R_DrawViewModel (void)
{
	...
	eglDepthRange (Q_GLDEPTHRANGE_MIN_0_0, Q_GLDEPTHRANGE_GUN_0_125);  GL_ErrorPrint;
	eglEnable (GL_DEPTH_TEST);  GL_ErrorPrint;
	eglEnable (GL_CULL_FACE);  GL_ErrorPrint;
	R_DrawAliasModel (currententity);
	eglDepthRange (Q_GLDEPTHRANGE_MIN_0_0, Q_GLDEPTHRANGE_MAX_0_5);  GL_ErrorPrint;
...
{
	const int size = 8; // cross length

	egl_Begin_Lines_3D_Array (GL_LINES); // handled
	egl_Add_Vertex_3D (origin[0]-size, origin[1], origin[2]);
	egl_Add_Vertex_3D (origin[0]+size, origin[1], origin[2]);
	egl_Add_Vertex_3D (origin[0], origin[1]-size, origin[2]);
	egl_Add_Vertex_3D (origin[0], origin[1]+size, origin[2]);
	egl_Add_Vertex_3D (origin[0], origin[1], origin[2]-size);
	egl_Add_Vertex_3D (origin[0], origin[1], origin[2]+size);
	egl_End_Lines_3D_Array ();

	//eglBegin (GL_LINES); // handled
...
{
#if 1 // Leave this one manual I guess.  I don't want to deal with it right now.
	egl_Begin_Untextured_Quads_3D_Elements (GL_QUAD_STRIP);
	//X_BEGIN_LINES_SELF ();  idx = _idx_base;
	
	egl_Add_Vertex_3D (mins[0], mins[1], mins[2]);
	egl_Add_Vertex_3D (mins[0], mins[1], maxs[2]);
		*idx++ = 0;	*idx++ = 1;

	egl_Add_Vertex_3D (maxs[0], mins[1], mins[2]);
	egl_Add_Vertex_3D (maxs[0], mins[1], maxs[2]);
		*idx++ = 0;	*idx++ = 2; // 8
		*idx++ = 1;	*idx++ = 3;
		*idx++ = 2;	*idx++ = 3;

	egl_Add_Vertex_3D (maxs[0], maxs[1], mins[2]);
	egl_Add_Vertex_3D (maxs[0], maxs[1], maxs[2]);
		*idx++ = 2;	*idx++ = 4; // + 6
		*idx++ = 3;	*idx++ = 5;
		*idx++ = 4;	*idx++ = 5;


	egl_Add_Vertex_3D (mins[0], maxs[1], mins[2]);
	egl_Add_Vertex_3D (mins[0], maxs[1], maxs[2]);
		*idx++ = 4;	*idx++ = 6; // + 6
		*idx++ = 5;	*idx++ = 7;
		*idx++ = 6;	*idx++ = 7;
	...
	glVertexPointer			(3 /*dimension*/, GL_FLOAT, /*stride in bytes*/ sizeof(floatsamx_t), &_ray_base[0].x);
	glDrawElements			(GL_LINES, /* count of indices, not triangles */ idx - _idx_base, GL_UNSIGNED_SHORT, _idx_base);
	
	glDisableClientState	(GL_VERTEX_ARRAY);																					
#else
	eglBegin (GL_QUAD_STRIP); // handled
...
{
	egl_Begin_Untextured_Quads_3D_Elements (GL_QUADS); // Series of QUADS

    // Front Face
    egl_Add_Untextured_Quad_Vert_3D_Element (mins[0], mins[1], maxs[2]); // 0 1 2 0 2 3 .. cw or ccw?
    egl_Add_Untextured_Quad_Vert_3D_Element (maxs[0], mins[1], maxs[2]);
    egl_Add_Untextured_Quad_Vert_3D_Element (maxs[0], maxs[1], maxs[2]);
    egl_Add_Untextured_Quad_Vert_3D_Element (mins[0], maxs[1], maxs[2]);
		egl_Complete_This_Untextured_Quad_3D_Element ();

    // Back Face
    egl_Add_Untextured_Quad_Vert_3D_Element (mins[0], mins[1], mins[2]);
    egl_Add_Untextured_Quad_Vert_3D_Element (mins[0], maxs[1], mins[2]);
    egl_Add_Untextured_Quad_Vert_3D_Element (maxs[0], maxs[1], mins[2]);
    egl_Add_Untextured_Quad_Vert_3D_Element (maxs[0], mins[1], mins[2]);
		egl_Complete_This_Untextured_Quad_3D_Element ();
    ...
    egl_Add_Untextured_Quad_Vert_3D_Element (maxs[0], mins[1], mins[2]);
    egl_Add_Untextured_Quad_Vert_3D_Element (maxs[0], mins[1], maxs[2]);
    egl_Add_Untextured_Quad_Vert_3D_Element (mins[0], mins[1], maxs[2]);
		egl_Complete_This_Untextured_Quad_3D_Element ();
	...
	egl_End_Untextured_Quads_3D_Elements ();
}
...
Point3D R_EmitSurfaceHighlight (entity_t* enty, msurface_t* surf, rgba4_t color, int style)
{
	...
	if (enty)
	{
		eglPushMatrix ();  GL_ErrorPrint;
		R_RotateForEntity (enty->origin, enty->angles);
	}

#ifndef PLATFORM_ANDROID // No can do
	if (style == OUTLINED_POLYGON)	// Set to lines
		glPolygonMode	(GL_FRONT_AND_BACK, GL_LINE);
#endif // !PLATFORM_ANDROID

	eglDisable (GL_TEXTURE_2D);  GL_ErrorPrint;
	eglEnable (GL_POLYGON_OFFSET_FILL);  GL_ErrorPrint;
	eglDisable (GL_CULL_FACE);  GL_ErrorPrint;
	eglColor4f (color[0], color[1], color[2], color[3]);  GL_ErrorPrint;
	eglDisable (GL_DEPTH_TEST);  GL_ErrorPrint;
	eglEnable (GL_BLEND);  GL_ErrorPrint;

	egl_Begin_Untextured_Polygon_3D_Array (GL_POLYGON);
	// eglBegin (GL_POLYGON); // handled
	...
	for (i = 0 ; i < surf->polys->numverts ; i ++, verts += VERTEXSIZE) {
		...
		egl_Add_VertexV_3D (verts);
		// eglVertex3fv (verts);
	}

	egl_End_Untextured_Polygon_3D_Array ();
	// eglEnd ();

	eglEnable (GL_TEXTURE_2D);  GL_ErrorPrint;
	eglDisable (GL_POLYGON_OFFSET_FILL);  GL_ErrorPrint;
	eglEnable (GL_CULL_FACE);  GL_ErrorPrint;
	eglColor4f (1, 1, 1, 1);  GL_ErrorPrint;
	eglEnable (GL_DEPTH_TEST);  GL_ErrorPrint;
	eglDisable (GL_BLEND);  GL_ErrorPrint;
...
void R_EmitBox (const vec3_t myColor, const vec3_t centerpoint, const vec3_t xyz_mins, const vec3_t xyz_maxs, const int bTopMost, const int bLines, const float Expandsize)
{
	...
	eglEnable			(GL_POLYGON_OFFSET_FILL);  GL_ErrorPrint;
	eglDisable			(GL_CULL_FACE);  GL_ErrorPrint;
	eglColor4fv			(myColor);  GL_ErrorPrint;

	if (myColor[3])	// If no alpha then it is solid
	{
		eglEnable		(GL_BLEND);  GL_ErrorPrint;
		eglDepthMask	(GL_FALSE);  GL_ErrorPrint;
	}
	...
	{
		eglDisable		(GL_BLEND);  GL_ErrorPrint;
		eglDepthMask	(GL_TRUE);  GL_ErrorPrint;
	}
...
void R_EmitCaption (vec3_t location, const char *caption, cbool backgrounded)
{
	...
	if (backgrounded)
	{
		...
		eglDepthRange (Q_GLDEPTHRANGE_GUN_0_125, Q_GLDEPTHRANGE_MAX_0_5 - 0.0125);   GL_ErrorPrint; // 0.3 to 0.5 ish
		eglColor4f (0,0,0,1);  GL_ErrorPrint;

		egl_Begin_Untextured_Solo_Quad_3D_Array (GL_QUADS); // 1 Quad
		//eglBegin (GL_QUADS); // Handled		

		// top left
		VectorMA (location, captionbox.top-2, up, point);
		VectorMA (point, captionbox.left+4, right, point);
		egl_Add_VertexV_3D (point);
		//eglVertex3fv (point);
		...
		egl_End_Untextured_Solo_Quad_3D_Array (); // GL_TRIANGLE_FAN rendering of 1 Quad
//		eglEnd ();
...
//#pragma message ("Baker: Depth range here ...")
	...
	eglDisable	(GL_CULL_FACE);  GL_ErrorPrint;
	eglEnable	(GL_ALPHA_TEST);  GL_ErrorPrint;

	eglDepthRange (Q_GLDEPTHRANGE_GUN_0_125, Q_GLDEPTHRANGE_MAX_0_5 - 0.25);  GL_ErrorPrint;
	...
	for (i = 0, x = 0, y =0; i < string_length; i++)
	{
		...
		x++;
		{
			...
			// Draw box
			egl_Begin_Textured_Solo_Quad_3D_Array (GL_QUADS); // 1 Quad
			//eglBegin (GL_QUADS); // Handled

			// top left
			VectorMA (location, charcoords.top, up, point);
			VectorMA (point, charcoords.left, right, point);
			egl_Add_Texco_VertexV_3D (s, t + 0.0625, point);
			// eglTexCoord2f (s, t + 0.0625); eglVertex3fv (point);

			// top, right
			VectorMA (location, charcoords.top, up, point);
			VectorMA (point, charcoords.right, right, point);
			egl_Add_Texco_VertexV_3D (s + 0.0625, t + 0.0625, point);
			// eglTexCoord2f (s + 0.0625, t + 0.0625); eglVertex3fv (point);

			// bottom right
			VectorMA (location, charcoords.bottom, up, point);
			VectorMA (point, charcoords.right, right, point);
			egl_Add_Texco_VertexV_3D (s + 0.0625, t, point);
			// eglTexCoord2f (s + 0.0625, t); eglVertex3fv (point);

			// bottom, left
			VectorMA (location, charcoords.bottom, up, point);
			VectorMA (point, charcoords.left, right, point);
			egl_Add_Texco_VertexV_3D (s, t, point);
			// eglTexCoord2f (s, t); eglVertex3fv (point);

			egl_End_Textured_Solo_Quad_3D_Array ();
			// eglEnd ();
		}
	}
...
void R_ShowBoundingBoxes (void)
{
	...
	glPolygonMode (GL_FRONT_AND_BACK, GL_LINE);
#endif // !PLATFORM_ANDROID
	GL_PolygonOffset (OFFSET_SHOWTRIS);  GL_ErrorPrint;
	eglDisable (GL_TEXTURE_2D);  GL_ErrorPrint;
	eglDisable (GL_CULL_FACE);  GL_ErrorPrint;
	eglColor3f (1,1,1);  GL_ErrorPrint;
...
//				continue; //don't draw if not in pvs
	...
	glPolygonMode (GL_FRONT_AND_BACK, GL_FILL);
#endif // ! PLATFORM_ANDROID
	GL_PolygonOffset (OFFSET_NONE);  GL_ErrorPrint;
	eglEnable (GL_DEPTH_TEST);  GL_ErrorPrint;
...
void R_DrawShadows (void)
{
	...
	{
		eglClearStencil (0);   GL_ErrorPrint;// Regular stencil shadows clear to 0
		eglClear(GL_STENCIL_BUFFER_BIT);  GL_ErrorPrint;
		eglStencilFunc(GL_EQUAL, 0, ~0);   GL_ErrorPrint;// Stencil Sky Fix - 2015 April 13 Per Spike
		eglStencilOp(GL_KEEP, GL_KEEP, GL_INCR);   GL_ErrorPrint;
		eglEnable(GL_STENCIL_TEST);  GL_ErrorPrint;
	}
	...
	if (renderer.gl_stencilbits DIRECT3D9_STENCIL_DISABLE_BLOCK_AND)
	{
		eglDisable(GL_STENCIL_TEST);  GL_ErrorPrint;
	}
...
//		Test indicates that it works right in determining whether to do any mirror stuff.  r_mirroralpha 1 = never, and if no mirror in frustum we don't either.
	...
	// PREPARE FOR MIRROR SURFACE DRAW
	{
		...
		eglDepthMask(GL_FALSE);  GL_ErrorPrint;
		eglColor4f (1,0,0,1);  GL_ErrorPrint;
		eglEnable (GL_BLEND);  GL_ErrorPrint;
		eglBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  GL_ErrorPrint;
		GL_DisableMultitexture ();
		eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_ADD);  GL_ErrorPrint;
#if 1		
		eglColorMask(0, 0, 0, 0);   GL_ErrorPrint; // DEPTH ONLY.  BLOCK ALL COLOR WRITING!
#endif
...

	{
		...
		eglColorMask(1, 1, 1, 1);  GL_ErrorPrint;
		GL_DisableMultitexture ();
		eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_REPLACE);  GL_ErrorPrint;
		eglColor4f (1, 1, 1, 1);  GL_ErrorPrint;
		eglDisable (GL_BLEND);  GL_ErrorPrint;
		eglDepthMask(GL_TRUE);  GL_ErrorPrint;
		eglBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  GL_ErrorPrint;
...
//	Con_SafePrintLinef ("Mirror %d", cl.r_framecount) ;//RANDOM_FLOAT * 100);
	...
	eglGetFloatv (GL_PROJECTION_MATRIX, frame.original_view.projection);  GL_ErrorPrint;	// Do these now.  Because gun kick or other stuff may have changed them a little.
	eglGetFloatv (GL_MODELVIEW_MATRIX, frame.original_view.modelview);	  GL_ErrorPrint;	//					Maybe???
...
//			VectorSet (r_refdef.viewangles, 27, 219, 0);
	...
	eglMatrixMode(GL_PROJECTION);  GL_ErrorPrint;
		switch (!!frame.mirror_plane->normal[2]) {
			case 1:		eglScalef ( 1, -1, 1);   GL_ErrorPrint;break;
			default:	eglScalef (-1,  1, 1);   GL_ErrorPrint;break;
			}
		eglGetFloatv (GL_PROJECTION_MATRIX, frame.mirror_view.projection);  GL_ErrorPrint;
	...
	eglMatrixMode(GL_MODELVIEW);  GL_ErrorPrint;
		...
		eglGetFloatv (GL_MODELVIEW_MATRIX, frame.mirror_view.modelview);  GL_ErrorPrint;

	// CAPABILITIES																	// Normally
	{
		eglCullFace(GL_FRONT);															  GL_ErrorPrint;// eglCullFace(GL_BACK);
		eglDepthRange (Q_GLDEPTHRANGE_MIN_MIRROR_0_5, Q_GLDEPTHRANGE_MAX_MIRROR_1_0);	  GL_ErrorPrint;// eglDepthRange (Q_GLDEPTHRANGE_MIN_0_0, Q_GLDEPTHRANGE_MAX_0_5);
		eglDepthFunc (GL_LEQUAL);   GL_ErrorPrint;// This is the normal.
	...
	{
		eglDepthRange (Q_GLDEPTHRANGE_MIN_0_0, Q_GLDEPTHRANGE_MAX_0_5);  GL_ErrorPrint;
		eglDepthFunc (GL_LEQUAL);  GL_ErrorPrint;
		eglCullFace(GL_BACK);  GL_ErrorPrint;
		frame.in_mirror_draw = false;
	}

	// RESTORE
	eglMatrixMode(GL_PROJECTION);   GL_ErrorPrint;// Set to projection matrix
	eglLoadMatrixf (frame.original_view.projection);  GL_ErrorPrint;
	eglMatrixMode(GL_MODELVIEW);  GL_ErrorPrint;
	eglLoadMatrixf (frame.original_view.modelview);  GL_ErrorPrint;
	...
	// PREPARE FOR MIRROR SURFACE DRAW
	{
		...
		eglColor4f (1,1,1, frame.mirroralpha);  GL_ErrorPrint;
		eglEnable (GL_BLEND);  GL_ErrorPrint;
		eglBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);  GL_ErrorPrint;
		GL_DisableMultitexture ();
		eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);  GL_ErrorPrint;
		frame.in_mirror_overlay = true;
	}
...
void R_RenderView (void)
{
	...
	if (gl_stereo.value)
	{
		...
		//render left eye (red)
		eglColorMask(1, 0, 0, 1);  GL_ErrorPrint;
		...
		eglClear (GL_DEPTH_BUFFER_BIT);  GL_ErrorPrint;
		eglColorMask(0, 1, 1, 1);  GL_ErrorPrint;
