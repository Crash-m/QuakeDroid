...
void R_RenderDlight (dlight_t *light)
{
	...
	egl_Begin_RGBf_Triangle_Fan_3D_Array (GL_TRIANGLE_FAN);
	for (i=0 ; i<3 ; i++)
		v[i] = light->origin[i] - vpn[i]*rad;
	
	egl_Add_RGBf_VertexV_3D_Array (0.2, 0.1, 0.0, v);
	
	for (i = 16 ; i >= 0 ; i--) {
		for (j = 0 ; j < 3 ; j++)
			v[j] = light->origin[j] + vright[j] * bubble_cos[i] * rad + vup[j] * bubble_sin[i] * rad;
		egl_Add_RGBf_VertexV_3D_Array (0, 0, 0, v);
	}
	
	egl_End_RGBf_Triangle_Fan_3D_Array ();
...
//		v[i] = light->origin[i] - vpn[i]*rad;
//
//	eglColor3f (0.2,0.1,0.0); eglVertex3fv (v);
//	eglColor3f (0,0,0);
//	for (i = 16 ; i >= 0 ; i--)
//	{
//		for (j = 0 ; j < 3 ; j++)
