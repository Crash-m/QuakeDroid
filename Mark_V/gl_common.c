...
static void GL_Evaluate_Renderer_CheckExtensions (void)
{
	...
	eglGetIntegerv (GL_MAX_TEXTURE_SIZE, &renderer.gl_max_texture_size);GL_ErrorPrint;
	...
	else if (vid.is_mobile || GL_ParseExtensionList(renderer.gl_extensions, "GL_ARB_multitexture"))
	{
		...
		renderer.GL_ClientActiveTexture = glClientActiveTexture;
#else
		...
		renderer.GL_ClientActiveTexture = (PFNGLCLIENTACTIVETEXTUREARBPROC) Vid_GL_GetProcAddress("glClientActiveTextureARB");
#endif
		
		if (PLATFORM_MOBILE || (renderer.GL_MTexCoord2fFunc && renderer.GL_SelectTextureFunc))
		{
	...
	if (GL_ParseExtensionList(renderer.gl_extensions, "GL_EXT_texture_filter_anisotropic"))
	{
		float test1,test2;
		GLuint tex;
//		alert ("GL_EXT_texture_filter_anisotropic");
		// test to make sure we really have control over it
		// 1.0 and 2.0 should always be legal values
		eglGenTextures(1, &tex);GL_ErrorPrint;
		eglBindTexture (GL_TEXTURE_2D, tex);GL_ErrorPrint;
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, 1.0f);GL_ErrorPrint;
		eglGetTexParameterfv (GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, &test1);GL_ErrorPrint;
		eglTexParameterf(GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, 2.0f);GL_ErrorPrint;
		eglGetTexParameterfv (GL_TEXTURE_2D, GL_TEXTURE_MAX_ANISOTROPY_EXT, &test2);GL_ErrorPrint;
		eglDeleteTextures(1, &tex);GL_ErrorPrint;
		...
		eglGetFloatv (GL_MAX_TEXTURE_MAX_ANISOTROPY_EXT, &renderer.gl_max_anisotropy);GL_ErrorPrint;
...
{
	eglClearColor (0.15, 0.15, 0.15, 0);  GL_ErrorPrint;//johnfitz -- originally 1,0,0,0
	eglCullFace(GL_BACK); GL_ErrorPrint;//johnfitz -- glquake used CCW with backwards culling -- let's do it right
	eglFrontFace(GL_CW); GL_ErrorPrint;//johnfitz -- glquake used CCW with backwards culling -- let's do it right
	eglEnable(GL_TEXTURE_2D);GL_ErrorPrint;
	eglEnable(GL_ALPHA_TEST);GL_ErrorPrint;
	eglAlphaFunc(GL_GREATER, 0.666);GL_ErrorPrint;

#if 0 //ndef PLATFORM_OPENGLES // Not available for OpenGLES
	eglPolygonMode (GL_FRONT_AND_BACK, GL_FILL);
#endif

	eglShadeModel (GL_FLAT);GL_ErrorPrint;
	eglHint (GL_PERSPECTIVE_CORRECTION_HINT, GL_NICEST); GL_ErrorPrint;//johnfitz
...
#ifdef GLQUAKE
	eglDepthRange (Q_GLDEPTHRANGE_MIN_0_0, Q_GLDEPTHRANGE_MAX_0_5);GL_ErrorPrint; // Baker: Half the possible Z range is still plenty. Mirror support.
#endif // GLQUAKE
	eglDepthFunc (GL_LEQUAL);GL_ErrorPrint; //johnfitz -- moved here becuase gl_ztrick is gone.
	if (renderer.gl_stencilbits) {
		eglClearStencil (0); GL_ErrorPrint;// Stencil Sky Fix - 2015 April 13 Per Spike
	}
...
///////////////////////////////////////////////////////////////////////////////
	...
	eglPixelStorei (GL_PACK_ALIGNMENT, 1);GL_ErrorPrint;
}
...
void GL_VID_SetMode_GL_Evaluate_Renderer (void)
{
	...
	renderer.gl_vendor = (const char *) eglGetString (GL_VENDOR);GL_ErrorPrint;
	renderer.gl_renderer = (const char *) eglGetString (GL_RENDERER);GL_ErrorPrint;
	renderer.gl_version = (const char *) eglGetString (GL_VERSION);GL_ErrorPrint;
	renderer.gl_extensions = (const char *) eglGetString (GL_EXTENSIONS);GL_ErrorPrint;
	...
	if (vid.direct3d == 8)
		...
		eglGetIntegerv(GL_STENCIL_BITS, &renderer.gl_stencilbits);  GL_ErrorPrint;
		if(!renderer.gl_stencilbits)
