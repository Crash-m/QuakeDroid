...
//	float			alpha; //johnfitz -- particle transparency
	...
	{
		egl_Begin_Textured_RGBA_Quads_3D_Elements (GL_QUADS);
		...
		for (p=active_particles ; p ; p=p->next)
		{
			...
			egl_Add_Texco_RGBAUB_VertexV_3D (/*texco*/ 0, 0, color, p->org);
			//eglTexCoord2f (0,0);
			//eglVertex3fv (p->org);

			
			VectorMA (p->org, scale, up, p_up);
			egl_Add_Texco_RGBAUB_VertexV_3D (/*texco*/ 0.5, 0, color, p_up);
			//eglTexCoord2f (0.5,0);
			//eglVertex3fv (p_up);

			
			VectorMA (p_up, scale, right, p_upright);
			egl_Add_Texco_RGBAUB_VertexV_3D (/*texco*/ 0.5, 0.5, color, p_upright);
			//eglTexCoord2f (0.5,0.5);
			//eglVertex3fv (p_upright);

			
			VectorMA (p->org, scale, right, p_right);
			egl_Add_Texco_RGBAUB_VertexV_3D (/*texco*/ 0, 0.5, color, p_right);
			//eglTexCoord2f (0,0.5);
			//eglVertex3fv (p_right);

			egl_Complete_This_Textured_RGBA_Quad_3D_Element ();
			rs_particles++; //johnfitz //FIXME: just use r_numparticles
		}
		egl_End_Textured_RGBA_Quads_3D_Elements ();
		//eglEnd ();

	}
	else //johnitz --  triangles save verts
	{
		egl_Begin_Textured_RGBA_Triangles_3D_Array (GL_TRIANGLES);
		//eglBegin (GL_TRIANGLES); // handled
		for (p=active_particles ; p ; p=p->next)
		{
			// hack a scale up to keep particles from disapearing
			scale = (p->org[0] - r_origin[0]) * vpn[0]
			...
			VectorMA (p->org, scale, up, p_up);
			egl_Add_Texco_RGBAUB_VertexV_3D (1, 0, color, p_up);
			//eglTexCoord2f (1,0);
			//eglVertex3fv (p_up);

			VectorMA (p->org, scale, right, p_right);
			egl_Add_Texco_RGBAUB_VertexV_3D (0, 1, color, p_right);
			//eglTexCoord2f (0,1);
			//eglVertex3fv (p_right);
		...
		//eglEnd ();
		egl_End_Textured_RGBA_Triangles_3D_Array ();
	}
...
void R_DrawParticles_ShowTris (void)
{
	...
	if (gl_quadparticles.value)
	{
		for (p=active_particles ; p ; p=p->next)
		{
			...
			egl_Add_VertexV_3D (p->org);
			//eglVertex3fv (p->org);

			VectorMA (p->org, scale, up, p_up);
			egl_Add_VertexV_3D (p_up);
			//eglVertex3fv (p_up);

			VectorMA (p_up, scale, right, p_upright);
			egl_Add_VertexV_3D (p_upright);
			//eglVertex3fv (p_upright);

			VectorMA (p->org, scale, right, p_right);
			egl_Add_VertexV_3D (p_right);
			//eglVertex3fv (p_right);
	...
	{
		egl_Begin_Untextured_Triangles_3D_Array (GL_TRIANGLES);
		//eglBegin (GL_TRIANGLES); // handled
		for (p=active_particles ; p ; p=p->next)
		{
			// hack a scale up to keep particles from disapearing
		...
		egl_End_Untextured_Triangles_3D_Array ();
//		eglEnd ();
	}
}
