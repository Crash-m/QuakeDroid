...
//	float			frow, fcol, size;
	...
	eglColor4f (1, 1, 1, 0.5);  GL_ErrorPrint;

	egl_Emit_Textured_Quad_2D (0, 0, 1, 1, x0, y0, x1, y1);
...
void Draw_CharacterQuad (int x, int y, char num)
{
	...
	egl_Emit_Textured_Quad_2D (fcol, frow, fcol + size, frow + size, x, y, x + 8, y + 8);
...
//#if 1
	//egl_Begin_Textured_Solo_Quad_2D_Array (GL_QUADS);
	//X_BEGIN_QUADS_2D (); // Reset

//	Draw_CharacterQuad (x, y, (char) num);

	//X_END_QUADS_2D ();
	//egl_End_Textured_Solo_Quad_2D_Array ();
...
void Draw_Pic (int x, int y, qpic_t *pic)
{
	...
	egl_Emit_Textured_Quad_2D (gl->sl, gl->tl, gl->sh, gl->th, x, y, x + pic->width, y + pic->height);
...
void Draw_ConsoleBackground (void)
{
	...
	if (alpha > 0.0)
	{
		...
		{
			eglEnable (GL_BLEND);  GL_ErrorPrint;
			eglColor4f (1,1,1,alpha);  GL_ErrorPrint;
			eglDisable (GL_ALPHA_TEST);  GL_ErrorPrint;
			eglTexEnvf(GL_TEXTURE_ENV, GL_TEXTURE_ENV_MODE, GL_MODULATE);  GL_ErrorPrint;
		}
...
void Draw_TileClear (int x, int y, int w, int h)
{
	...
	egl_Emit_Textured_Quad_2D (x/64.0, y/64.0, (x+w)/64.0, (y+h)/64.0, x, y, x + w, y + h);
...
void Draw_Triangle_Corner (int x, int y, int wh, int _wh, int c)
{
	...
	eglEnable (GL_BLEND);   GL_ErrorPrint;//johnfitz -- for alpha
	eglDisable (GL_ALPHA_TEST);   GL_ErrorPrint;//johnfitz -- for alpha
	//eglColor4f (pal[c*4]/255.0, pal[c*4+1]/255.0, pal[c*4+2]/255.0, alpha); //johnfitz -- added alpha
	eglColor4ub (pal[c*4], pal[c*4+1], pal[c*4+2], alpha * 255.0);   GL_ErrorPrint;//johnfitz -- added alpha

	egl_Emit_Untextured_Tri_2D (x, y, x + w, y, x, y + h);
	//eglBegin (GL_TRIANGLE_FAN); // handled
	//eglVertex2f (x,y);
	//eglVertex2f (x+w, y);
	//eglVertex2f (x, y+h);
	//eglEnd ();
	...
	eglColor3f (1,1,1);  GL_ErrorPrint;
	eglDisable (GL_BLEND);   GL_ErrorPrint;//johnfitz -- for alpha
	eglEnable (GL_ALPHA_TEST);  GL_ErrorPrint; //johnfitz -- for alpha
...
void Draw_Fill (int x, int y, int w, int h, int c, float alpha)
{
	...
	egl_Emit_Untextured_Quad_2D (x, y, x + w, y + h);
...
void Draw_FadeScreen (void)
{
	...
	eglEnable (GL_BLEND);  GL_ErrorPrint;
	eglBlendFunc (GL_SRC_ALPHA, GL_ONE_MINUS_SRC_ALPHA);   GL_ErrorPrint;// Baker: ha!  This was "random"
	eglDisable (GL_ALPHA_TEST);  GL_ErrorPrint;
	eglDisable (GL_TEXTURE_2D);  GL_ErrorPrint;

	eglColor4f (0, 0, 0, 0.5);  GL_ErrorPrint;
	
	egl_Emit_Untextured_Quad_2D (0, 0, clwidth, clheight);
...
//if (newcanvas == CANVAS_MENU_INTERMISSION_TEXT ) newcanvas = CANVAS_MENU;
	...
	{
	case CANVAS_DEFAULT:
		eglOrtho (0, clwidth, clheight, 0, -99999, 99999);  GL_ErrorPrint;
		eglViewport (clx, cly, clwidth, clheight);  GL_ErrorPrint;
		break;
	case CANVAS_DEFAULT_CONSCALE:
		eglOrtho (0, vid.conwidth, vid.conheight, 0, -99999, 99999);  GL_ErrorPrint;
		eglViewport (clx, cly, clwidth, clheight);  GL_ErrorPrint;
		break;
	case CANVAS_CONSOLE:
		lines = vid.conheight - (console1.visible_lines_conscale);
		eglOrtho (0, vid.conwidth, vid.conheight + lines, lines, -99999, 99999);  GL_ErrorPrint;
		eglViewport (clx, cly, clwidth, clheight);  GL_ErrorPrint;
		break;
	...
	case CANVAS_MENU:
		...
		eglOrtho (0, 320, 200, 0, -99999, 99999);  GL_ErrorPrint;
		...
		if (newcanvas ==  CANVAS_MENU) {
			...
			focus0.menu_viewport[0] = clx + (clwidth - 320*s) / 2; 
...
#else
			eglGetIntegerv	(GL_VIEWPORT, focus0.menu_viewport);  GL_ErrorPrint;

			eglGetFloatv	(GL_MODELVIEW_MATRIX, focus0.menu_projection.m16);   GL_ErrorPrint;// Store off the model view matrix.  (r_world_matrix)
			eglGetFloatv	(GL_PROJECTION_MATRIX, focus0.menu_modelview.m16);   GL_ErrorPrint;// Store off the model view matrix.  (r_world_matrix)
#endif
		}
	...
	case CANVAS_MENU_INTERMISSION_TEXT: // Increased size a bit.
		...
		s = CLAMP (1.0, vid.menu_scale /*gl_menuscale.value*/, s);
		{
			...
			eglOrtho (0, 320 /* x_width /s */ , y_height_scaled, 0, -99999, 99999);  GL_ErrorPrint;
			eglViewport (clx + x_offset, cly + (clheight - y_height_scaled *s) / 2, x_width, y_height_scaled * s);  GL_ErrorPrint;
		}
		break;
	case CANVAS_SBAR:
		// HERE AUTO
		...
		{
			eglOrtho (0, clwidth / s, 48, 0, -99999, 99999);  GL_ErrorPrint;
			eglViewport (clx, cly, clwidth, 48*s);  GL_ErrorPrint;
			// Baker: This would actually need translated to canvas default.
			RECT_SET (focus0.touch_sbar, clx, cly, clwidth, 48*s);
		}
		else
		{
			eglOrtho (0, 320, 48, 0, -99999, 99999);  GL_ErrorPrint;
			eglViewport (clx + (clwidth - 320*s) / 2, cly, 320*s, 48*s);  GL_ErrorPrint;
			// Baker: This would actually need translated to canvas default.
			RECT_SET (focus0.touch_sbar, clx + (clwidth - 320*s) / 2, vid.screen.height - cly, 320*s, 48*s);
		}
		break;
	case CANVAS_WARPIMAGE:
		eglOrtho (0, 128, 0, 128, -99999, 99999);  GL_ErrorPrint;
		eglViewport (clx, cly + clheight - gl_warpimagesize, gl_warpimagesize, gl_warpimagesize);  GL_ErrorPrint;
		break;
	case CANVAS_CROSSHAIR: //0,0 is center of viewport
		s = CLAMP (0.10, gl_crosshairscale.value, 10.0);
		eglOrtho (scr_vrect.width/-2.0f/s, scr_vrect.width/2.0f/s, scr_vrect.height/2.0f/s, scr_vrect.height/-2.0f/s, -99999, 99999);  GL_ErrorPrint;
		eglViewport (clx + scr_vrect.x, clheight - scr_vrect.y - scr_vrect.height, scr_vrect.width & ~1, scr_vrect.height & ~1);  GL_ErrorPrint;
		break;
	case CANVAS_BOTTOMLEFT: //used by devstats
		s = (float)clwidth/vid.conwidth; //use console scale
		eglOrtho (0, 320, 200, 0, -99999, 99999);  GL_ErrorPrint;
		eglViewport (clx, cly, 320 * s, 200 * s);  GL_ErrorPrint;
		break;
...
*/
	case CANVAS_TOPRIGHT: //used by disc
		s = 1;
		eglOrtho (0, 320, 200, 0, -99999, 99999);  GL_ErrorPrint;
		eglViewport (clx + clwidth-320 * s, cly + clheight-200 * s, 320 * s, 200 * s);  GL_ErrorPrint;
		break;

	case CANVAS_TOPLEFT: //used by disc
		s = 1;
		eglOrtho (0, 320, 200, 0, -99999, 99999);  GL_ErrorPrint;
		eglViewport (clx, cly + clheight-200 * s, 320 * s, 200 * s);  GL_ErrorPrint;
		break;
